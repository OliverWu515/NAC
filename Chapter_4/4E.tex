...
则在下述集合
\begin{equation}
    E = \left\{(x,\tilde{\theta}): \| x \| \leq \frac{d_{\max}}{k} = e_{0}\right\}\label{dead-zone}
\end{equation}
...
并且\begin{equation}
    ...
    \leq - \sigma\tilde{\theta}{\tilde{\theta}}^{2} + \sigma\left(\frac{{\tilde{\theta}}^{2}}{2} + \frac{\theta^{2}}{2}\right)
...
可见为了使$V$收敛迅速且接近于$0$，我们欲使\(\beta\)较大且\(c\)较小。对于前者，结合$\beta$的表达式，可取\(k,\sigma\gamma\)都较大；对于后者，结合$c$的表达式，可取\(k\)较大且\(\sigma\)较小。
...
\begin{table}[htbp]
  ...
  \hline
   开环被控对象 & $\dot{x} =  A x + B \Lambda \left( u + \Theta^\mathrm{T} \Phi(x) \right) + d(t)$\\
    ...
    Lyapunov 公式 & $P A_{\ensuremath{\operatorname{ref}}} + A_{\ensuremath{\operatorname{ref}}}^{\mathrm{T}} P = -Q < 0$\\
