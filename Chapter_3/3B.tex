...
\begin{definition}[非自治系统的稳定性]
 系统 \eqref{freeofnonauto} 的平衡点 $x = 0$是
  \begin{itemize}[leftmargin=1em]
    ...
    \item {\bf 渐近稳定的（asymptotically stable，AS）}\index{稳定的（stable）!渐近$\sim$（asymptotically）}，若其是稳定的，且存在正数
     $c = c (t_0)$ 使得
    \begin{equation}
      \forall \| x (t_0) \| < c, \lim_{t \rightarrow \infty} x (t) = 0 \label{Nonauto:astable:def} .
    \end{equation}
    \item {\bf 一致渐近稳定的（uniformly asymptotically stable，UAS）}，\index{稳定的（stable）!一致渐近$\sim$（uniformly asymptotically）}若其为一致稳定的，且存在一正常数 $c$（与$t_0$无关）使得 \eqref{Nonauto:astable:def} 成立；换言之，对任意$\eta>0$，都存在$T=T(\eta)>0$使得
